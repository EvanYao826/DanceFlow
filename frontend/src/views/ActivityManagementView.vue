...
const router = useRouter(); const route = useRoute(); const records = ref<Activity[]>([]); const loading = ref(true)
const labels: Record<string, string> = { DRAFT: '草稿', PUBLISHED: '已发布', CLOSED: '已关闭', FINISHED: '已完成', CANCELLED: '已取消' }
const filtered = computed(() => { const keyword = String(route.query.keyword || '').toLowerCase(); return records.value.filter(item => (!keyword || `${item.title} ${item.location}`.toLowerCase().includes(keyword)) && (!route.query.type || item.activityType === route.query.type) && (!route.query.status || item.status === route.query.status)) })
