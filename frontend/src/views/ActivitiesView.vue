...
<template>
  <main class="activities-page"><header class="topbar"><BrandLogo variant="purple" :height="34" to="/" /><nav><el-link @click="router.push('/club')">社团中心</el-link><el-link @click="router.push('/courses')">课程</el-link><el-link v-if="auth.isLoggedIn" @click="router.push('/profile/applications')">我的报名</el-link><el-link v-if="auth.isLoggedIn" @click="router.push('/courses/my')">我的学习</el-link><el-link v-if="auth.isLoggedIn" @click="auth.logout(); router.push('/login')">退出登录</el-link><el-link v-else @click="router.push('/login')">登录</el-link></nav></header>
